...
class TX10pCalculator(TemperaturePercentileCalculator):
    """
    Calculator for TX10p indicator: Cold days percentage.
    
    TX10p represents the percentage of days with maximum temperature < 10th percentile
    of the base period (1981-2010). This indicator measures the frequency of cold days.
    """

    INDICATOR_CODE = "TX10p"
    ...
    def calculate_annual(self) -> bool:
        """
        Calculate annual TX10p values.
        
        This method calculates the percentage of days with maximum temperature
        below the 10th percentile for each year in the specified date range.
        
        Returns:
            bool: True if calculation was successful, False otherwise
        """
        try:
            info("Starting TX10p annual calculation",
                 component="tx10p_calculator",
            ...
            if not percentiles_dict or 10 not in percentiles_dict:
                error("Failed to get base period 10th percentile",
                      component="tx10p_calculator")
                return False
            
            percentile_10 = percentiles_dict[10]
            ...
            if not datasets:
                error("Failed to get datasets for TX10p calculation",
                      component="tx10p_calculator")
                return False
            
            # Calculate TX10p for each year in parallel
            results = {}
            max_workers = int(os.getenv('MAX_PARALLEL_DOWNLOADS', 4))
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_year = {
                    executor.submit(self._calculate_tx10p_for_year, year, dataset, percentile_10): year
                ...
                for future in as_completed(future_to_year):
                    ...
                    try:
                        ...
                        if result is not None:
                            results[year] = result
                            info("TX10p calculated for year",
                                 component="tx10p_calculator",
                                 year=year)
                    except Exception as e:
                        error("Failed to calculate TX10p for year",
                              component="tx10p_calculator",
                              year=year,
                              error=str(e))
            ...
            # Save results
            if results:
                success = self._save_tx10p_results(results, datasets)
                ...
                error("No TX10p results calculated",
                      component="tx10p_calculator")
                return False
                
        except Exception as e:
            error("Failed to calculate TX10p annual values",
                  component="tx10p_calculator",
    ...
    def _calculate_tx10p_for_year(self, year: int, dataset: xr.Dataset, percentile_10: np.ndarray) -> Optional[np.ndarray]:
        """
        Calculate TX10p values for a specific year using the base period percentile.
        
        Args:
            year: Year to calculate TX10p for
            dataset: xarray Dataset with daily temperature data
            percentile_10: 2D array with 10th percentile values for each pixel
            
        Returns:
            numpy array with TX10p percentage values, or None if calculation fails
        """
        try:
            info("Calculating TX10p for year",
                 component="tx10p_calculator",
                 year=year,
            ...
            # Calculate TX10p for each pixel
            tx10p_values = self._calculate_cold_days_percentage(temp_values, percentile_10)
            
            # Handle cases where all values were NaN
            all_nan_mask = np.all(np.isnan(temp_values), axis=0)
            tx10p_values[all_nan_mask] = np.nan
            
            info("TX10p calculation completed for year",
                 component="tx10p_calculator",
                 year=year,
                 max_tx10p=np.nanmax(tx10p_values),
                 min_tx10p=np.nanmin(tx10p_values),
                 mean_tx10p=np.nanmean(tx10p_values))
    ...
    def _calculate_cold_days_percentage(self, temp_values: np.ndarray, percentile_10: np.ndarray) -> np.ndarray:
        """
        Calculate percentage of days with temperature below 10th percentile for each pixel.
        
        Args:
            temp_values: 3D array (time, lat, lon) with temperature values
            percentile_10: 2D array (lat, lon) with 10th percentile values
            
        Returns:
            2D array (lat, lon) with percentage values
        """
        time_steps, height, width = temp_values.shape
        tx10p_result = np.zeros((height, width), dtype=np.float32)
        
        # Process each pixel
        for i in range(height):
            for j in range(width):
                pixel_temp = temp_values[:, i, j]
                pixel_percentile = percentile_10[i, j]
                
                # Skip if percentile is NaN or all values are NaN
                if np.isnan(pixel_percentile) or np.all(np.isnan(pixel_temp)):
                    tx10p_result[i, j] = np.nan
                    continue
                ...
                # Count days below 10th percentile
                cold_days = np.sum(valid_temp < pixel_percentile)
                total_days = len(valid_temp)
                
                # Calculate percentage
                if total_days > 0:
                    tx10p_result[i, j] = (cold_days / total_days) * 100.0
                else:
                    tx10p_result[i, j] = np.nan
        
        return tx10p_result

    def _save_tx10p_results(self, results: dict, datasets: dict) -> bool:
        """
        Save TX10p calculation results to GeoTIFF files.
        
        Args:
            results: Dictionary mapping years to TX10p arrays
        ...
        """
        try:
            info("Saving TX10p results",
                 component="tx10p_calculator",
                 year_count=len(results))
            
            # For each year, save as GeoTIFF
            for year, tx10p_data in results.items():
                ...
                self._save_as_geotiff(tx10p_data, output_path, year, dataset)
                
                info("TX10p result saved",
                     component="tx10p_calculator",
                     year=year,
            ...
            error("Failed to save TX10p results",
                  component="tx10p_calculator",
                  error=str(e))
            return False
    ...
    def _save_as_geotiff(self, data: np.ndarray, output_path: Path, year: int, dataset: xr.Dataset):
        ...
        Args:
            data: 2D numpy array with TX10p values
        ...
        try:
            ...
            with rasterio.open(
                ...
                dst.update_tags(
                    INDICATOR='TX10p',
                    YEAR=str(year),
                    DESCRIPTION=f'Percentage of days with maximum temperature < 10th percentile of base period ({self.base_period_start}-{self.base_period_end})',
                    UNITS='percent',
                    BASE_PERIOD=f'{self.base_period_start}-{self.base_period_end}',
                    CREATED=datetime.now().isoformat()
                )
                
            info("TX10p data saved as GeoTIFF",
                 component="tx10p_calculator",
            ...
            error("Failed to save TX10p data as GeoTIFF",
                  component="tx10p_calculator",
                  output_path=str(output_path),
                  year=year,
                  error=str(e))
    ...
    def calculate_monthly(self) -> bool:
        """
        Monthly TX10p calculation (not implemented yet).

        This would calculate the percentage of cold days for each month.
        """
        warning("Monthly TX10p calculation not implemented",
               component="tx10p_calculator",
               indicator_code=self.INDICATOR_CODE)
        return False


class TX10pDataProcessor:
    """
    Helper class for TX10p data processing operations.
    ...
    @staticmethod
    def calculate_base_period_percentile(daily_data: xr.Dataset, percentile: float = 10.0) -> xr.Dataset:
        """
        Calculate percentile for base period daily temperature data.
        
        Args:
            daily_data: Daily temperature dataset for base period
            percentile: Percentile value to calculate (default 10.0)
    ...
    @staticmethod
    def calculate_percentage_below_threshold(daily_data: xr.Dataset, thresholds: xr.Dataset) -> xr.Dataset:
        """
        Calculate percentage of days below threshold for each year.
        ...
        # 2. Count days below threshold for each year
